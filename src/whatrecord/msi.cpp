...
void inputConstruct(inputData **ppvt);
void inputDestruct(inputData * const pvt);
void inputAddPath(inputData * const pvt, const char * const pval);
void inputBegin(inputData * const pvt, const char * const fileName);
char *inputNextLine(inputData * const pvt);
void inputNewIncludeFile(inputData * const pvt, const char * const name);
void inputErrPrint(const inputData * const pvt);

/* Module to read the substitution file */
typedef struct subInfo subInfo;

void substituteOpen(subInfo **ppvt, const std::string& substitutionName);
void substituteDestruct(subInfo * const pvt);
bool substituteGetNextSet(subInfo * const pvt, char **filename);
bool substituteGetGlobalSet(subInfo * const pvt);
const char *substituteGetReplacements(subInfo * const pvt);
const char *substituteGetGlobalReplacements(subInfo * const pvt);

/* Forward references to local routines */
void usageExit(const int status);
void abortExit(const int status);
void addMacroReplacements(MAC_HANDLE * const macPvt,
                                 const char * const pval);
void makeSubstitutions(inputData * const inputPvt,
...
void inputOpenFile(inputData *pinputData, const char * const filename);
void inputCloseFile(inputData *pinputData);
void inputCloseAllFiles(inputData *pinputData);
...
void inputDestruct(inputData * const pinputData)
{
...

void inputBegin(inputData * const pinputData, const char * const fileName)
{
    ENTER;
    inputCloseAllFiles(pinputData);
...
char *inputNextLine(inputData * const pinputData)
{
...
void inputNewIncludeFile(inputData * const pinputData,
...
void inputErrPrint(const inputData *const pinputData)
{
    ENTER;
...
char *subGetNextLine(subFile *psubFile);
tokenType subGetNextToken(subFile *psubFile);
void subFileErrPrint(subFile *psubFile, const char * message);
void freeSubFile(subInfo *psubInfo);
void freePattern(subInfo *psubInfo);
void catMacroReplacements(subInfo *psubInfo,const char *value);
...

void substituteDestruct(subInfo * const psubInfo)
{
    ENTER;
...
bool substituteGetGlobalSet(subInfo * const psubInfo)
{
    subFile *psubFile = psubInfo->psubFile;
...
bool substituteGetNextSet(subInfo * const psubInfo,char **filename)
{
    subFile     *psubFile = psubInfo->psubFile;
...

const char *substituteGetGlobalReplacements(subInfo * const psubInfo)
{
    subFile     *psubFile = psubInfo->psubFile;
...
const char *substituteGetReplacements(subInfo * const psubInfo)
{
    subFile     *psubFile = psubInfo->psubFile;
