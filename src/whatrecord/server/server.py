...
from whatrecord import graph
from whatrecord.server import html as html_mod
from whatrecord.server import static
from whatrecord.shell import load_multiple_startup_scripts
...
class ServerState:
    def __init__(self, startup_scripts):
        self.container = load_multiple_startup_scripts(
            *startup_scripts
        )
    ...
    def get_matching_pvs(self, glob_str: str) -> Tuple[str, ...]:
        ...
        return tuple(
            pv_name for pv_name in sorted(self.database)
            if regex.match(pv_name)
        )
...
class ServerHandler:
    ...
    async def api_pv_get_info(self, request: web.Request):
        ...
        info = {
            pv_name: self.state.container.whatrec(pv_name) or []
    ...
    async def api_ioc_info(self, request: web.Request):
        ...
        return web.json_response(
            dataclasses.asdict(script_info)
        )
